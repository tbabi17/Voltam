<?
	class crm_db {
        ...
        var $myconn;
		var $disable = true;
		var $crm_url = "http://localhost:85/svn/crm/";
	...
	class crm_user extends crm_task{
		var $basic_fields = "id,owner,password,fullName,phone,section,position,team,manager,gmailAccount,user_type,company,user_level,permission,partner,mon,thue,wed,thur,fri,sat,sun,warehouse_id,msg,pricetag";
	}
	...
	class crm {		
		...
		function crm_product_xml_list($item) {
			...
			$dir = $item->get("dir");
			$team = $item->get("where");
			file_put_contents("c:\\where.txt", $team);
			if($team == "хон"){
				$q = "SELECT * FROM crm_products";
			}	
			else{
				$q = "SELECT * FROM crm_products WHERE product_type<>'хон'";
			}
		...
		function crm_product_available_xml_list($item) {
			...
			if ($user_dir == "retail") {			
				...
				$q = "select product_id,total as last from crm_product_available";
		...
		function crm_owner_service_product_xml_list($item) {
			...
			if (strlen($service_id) > 0 && $service_id > 0)
				...
				if ($user_dir == "retail") {
					...
					if ($param == "lease") 
						$q = "SELECT product_id,sum(incoming) as qty, sum(amount) as amount,price,type,deposit FROM crm_balance WHERE deposit='lease' and product_id>0 and owner='$logged' and date(_date)='$date' and date(closing_date)<>'$date' and crm_id not in (select crm_id from crm_customer where _class='AGENT' and owner='$logged') GROUP by deposit,product_id,type,price ORDER BY $sort $dir";
					else
					if ($param == "payment") 
	/*					$q = "SELECT product_id,sum(incoming) as qty, sum(amount) as amount,price,type,deposit FROM crm_balance WHERE deposit='lease' and product_id>0 and owner='$logged' and date(closing_date)='$date' and crm_id not in (select crm_id from crm_customer where _class='AGENT' and owner='$logged') GROUP by deposit,product_id,type,price ORDER BY $sort $dir";*/
						$q = "SELECT product_id,sum(incoming) as qty, sum(amount) as amount,price,type,deposit FROM crm_balance WHERE deposit='lease'  and owner='$logged' and date(_date)='$date' and crm_id not in (select crm_id from crm_customer where _class='AGENT' and owner='$logged') GROUP by deposit,product_id,type,price ORDER BY $sort $dir";
					else
					if ($param == "total") 
						$q = "SELECT product_id,sum(incoming) as qty, sum(amount) as amount,price,'' as type,'' as deposit FROM crm_balance WHERE (deposit='cash' or deposit='lease') and product_id>0 and owner='$logged' and (date(closing_date)='$date' or date(_date)='$date') and crm_id not in (select crm_id from crm_customer where _class='AGENT' and owner='$logged') GROUP by product_id,price ORDER BY $sort $dir";
					else
					if ($param == "must") 
						/*$q = "SELECT product_id,sum(incoming) as qty, sum(amount) as amount,price,'' as type,'' as deposit FROM crm_balance WHERE (deposit='cash') and owner='$logged' and (date(closing_date)='$date' or date(_date)='$date') and crm_id not in (select crm_id from crm_customer where _class='AGENT' and owner='$logged') GROUP by product_id,price";*/
						$q="SELECT product_id,sum(incoming) as qty, sum(amount) as amount,price,type,deposit FROM crm_balance WHERE (deposit='lease') and product_id>0 and owner='$logged' and date(closing_date)='$date' and crm_id not in (select crm_id from crm_customer where _class='AGENT' and owner='$logged') GROUP by deposit,product_id,type,price  union SELECT product_id,sum(incoming) as qty, sum(amount) as amount,price,'' as type,'' as deposit FROM crm_balance WHERE (deposit='cash') and owner='$logged' and product_id>0 and (date(closing_date)='$date' or date(_date)='$date') and crm_id not in (select crm_id from crm_customer where _class='AGENT' and owner='$logged') GROUP by product_id,price";
		...
		function crm_owner_service_customer_xml_list($item) {
			...
			$user_dir = $this->crm_user_direction("owner=$logged");
			if ($user_dir == "retail")
				$q = "SELECT crm_id, sum(amount) as amount,GROUP_CONCAT(DISTINCT warehouse_id ORDER BY warehouse_id) as warehouse,type FROM crm_deal_products WHERE userCode='$logged' and date(_date)='$date' and crm_id not in (select crm_id from crm_customer where _class='AGENT' and owner='$logged') GROUP by crm_id";
			else
				$q = "SELECT crm_id,sum(qty) as qty, sum(amount) as amount,GROUP_CONCAT(DISTINCT warehouse_id ORDER BY warehouse_id) as warehouse,type FROM crm_deal_products WHERE userCode='$logged' and date(_date)='$date' GROUP by crm_id";
			
			$result = $this->query_result($q, " ORDER BY $sort $dir");
			while ($row = mysql_fetch_array($result)) {
				$record = new crm_service_customer();
				$row['crm_name'] = $this->crm_customer_name_simple($row['crm_id']).' '.$row['type'];
				$record->init($row);
				$array[$count] = $record;
...
/*			$q = "select concat(cast(id as char), '_t') as id,crm_id,subject,(duedate) as days, duetime as times,priority, 'task' as work_type, descr, task_status as status,owner,case_id,deal_id,campaign,'' as phone,_date,userCode from crm_tasks $where and 1=$e $date_where
		...
		function crm_storage_list($item) {
			...
			$where = $this->crm_where($item, $record->basic_fields);
			$count = $this->num_rows($q, $where." and deposit='storage' and warehouse_id=1 GROUP BY product_id");
			$result = $this->query_result($q, $where." and deposit='storage' and warehouse_id=1 GROUP BY product_id LIMIT $start,$end");
			...
			$json = substr($json, 0, -1);
			return '{"results":'.$count.',"items":['.$json.'],"query":"'.$q." ".$where." and deposit='storage' and warehouse_id=1 GROUP BY product_id LIMIT $start,$end".'"}';
		}
		...
		function crm_product_list($item) {
			...
			$wr = "";
			if ($user_level == 2) {
				$wr = " and 1=1 and product_type='хон' ";
			}

			$result = $this->query_result($q, $where.$wr." ORDER BY $sort $dir LIMIT $start,$end");
			$_SESSION['Products'] = $q.$where.$wr." ORDER BY $sort $dir";
			while ($row = mysql_fetch_array($result)) {
				$record = new crm_product();
				$record->init($row);
				$array[$count] = $record;
		...
		function crm_service_list_xml_before() {
			...
			while ($row_two = mysql_fetch_array($result_two)) {
					$id = $row_two['id'];
					$query = "update crm_deal_products set service_id=ifnull((select service_id from crm_services_tmp where subject=crm_deal_products.descr and userCode=crm_deal_products.userCode and flag>0 and crm_id=crm_deal_products.crm_id limit 0,1),0) where id=$id";
					mysql_query($query, $this->db->conn());
			}
		}
		...
		function crm_service_list_before() {
			...
			while ($row6 = mysql_fetch_array($result)) {
				if ($last_id < intval($row6['new_id'])) {
					...
					$q = "SELECT service_id FROM crm_services where flag>0 and _date>=SUBDATE(now(), INTERVAL 1 day) and (service_revenue<>(select service_revenue from crm_service_temp where descr=subject) or service_debt<>(select service_debt from crm_service_temp where descr=subject))";
					$result = $this->query_result($q, "");
					while ($row = mysql_fetch_array($result)) {
		...
		function crm_service_list($item) {
			...
			if ($team == "erdenet") 
				$where .= " and userCode='erdenet@voltam'";
			else			
			if ($team == "darkhan") 
				$where .= " and userCode='darkhan@voltam'";
			else
		...
		function crm_user_planning_sub($product_id, $owner, $start_date, $end_date) {
			$result = $this->query_result("(select sum(qty) as q,sum(amount) as a from crm_deal_products_for_plan where product_id=$product_id and userCode='$owner' and _date between '$start_date' and DATE_ADD('$end_date',INTERVAL 1 DAY) and crm_id not in (select crm_id from crm_customer where _class='AGENT'))", "");
			$return = "0,0";
			while ($row = mysql_fetch_array($result)) {
				$return = $row['q'].",".$row['a'];
			}
		...
		function crm_prepare_tmp_deal_products($views) {
			$query = "delete from crm_deal_products_for_plan";
			mysql_query($query, $this->db->conn());
			
			$query = "INSERT INTO crm_deal_products_for_plan (id,deal_id,service_id,product_id,precent,pty,qty,price,amount,type,crm_id,warehouse_id,_date,descr,userCode,unit_size,flag) select id,deal_id,service_id,product_id,precent,pty,qty,price,amount,type,crm_id,warehouse_id,_date,descr,userCode,unit_size,flag from crm_deal_products where YEAR('$views')=YEAR(_date) and MONTH('$views')=MONTH(_date)";
			mysql_query($query, $this->db->conn());
		}
...
//			$where .= $this->crm_view_query($item);
			...
			if ($this->startsWith($views, "201"))
				...
				(select id,deal_id,service_id,product_id,precent,pty,qty,price,amount,type,crm_id,warehouse_id,_date,descr,userCode,unit_size,flag from crm_deal_products where YEAR('$views')=YEAR(_date) and MONTH('$views')=MONTH(_date))";
...
//			$where .= $this->crm_view_query($item);
			...
			if ($this->startsWith($views, "201"))
				$wq = " and '$views' between start_date and end_date ORDER BY _class $dir";
				$this->crm_prepare_tmp_deal_products($views);
			$count = $this->num_rows($q, $where.$wq);
			$result = $this->query_result($q, $where.$wq." ");
			while ($row = mysql_fetch_array($result)) {
				$record = new crm_goal();
...
//				$row['performTheshold'] = 0;
		...
		function crm_customer_monthly_report_xml_list($item) {
			...
			$result = $this->query_result($q, " WHERE crm_id='$wh[0]' and month('$wh[1]')=month(_date) and year('$wh[1]')=year(_date) GROUP BY product_id order by (select _class from crm_products where product_id=crm_balance.product_id)");
			while ($row = mysql_fetch_array($result)) {
				$v = $this->crm_product_name_simple($row['product_id']);
				$vw = explode('|', $v);
				$row['product_name'] = $vw[0];
		...
		function crm_test_xml_list($item) {
			...
			$client = new SoapClient('http://202.70.43.26/pos/Service1.svc?wsdl');
		...
		function crm_report_product_list($item) {
			...
			$count = $this->num_rows($q, $where);
			$result = $this->query_result($q, $where." and deposit='sales' group by product_id ORDER BY _class ASC");
			echo $q. $where." and deposit='sales' group by product_id ORDER BY _class ASC";
			$_SESSION['CRM_REPORT_PRODUCT'] = $q." ".$where." and deposit='sales' group by product_id ORDER BY _class ASC";

			while ($row = mysql_fetch_array($result)) {
				$record = new crm_report_product();
				
				$gg = $this->crm_product_planning_summary($row['product_id'], $item);
				$row['g_qty'] = $gg['qty'];
				$row['g_amount'] = $gg['amount'];
				...
				$row['unit_size'] = $vw[3];
				$row['g_pty'] = doubleval($row['g_qty']) / 10;
				$row['product_barcode'] = $vw[4];
				
				$gamount = doubleval($row['g_amount']);
				if ($gamount == 0) {
					$row['sku'] = 0;
				}
				else
					$row['sku'] = doubleval($row['amount']) * 100/$gamount;
				$record->init($row);
				$array[$count] = $record;
				$json .= $record->json().",";
		...
		function crm_customer_sale_info($crm_id, $item) {	
			...
			$end_date = $item->get("end_date");
			$array = array();
			$result = $this->query_result("SELECT deposit,sum(amount) as amount from crm_balance WHERE crm_id=".$crm_id." and _date between '$start_date' and '$end_date' group by deposit", "");
			while ($row = mysql_fetch_array($result)) {
		...
		function crm_customer_entry_info($crm_id, $item) {	
			...
			$end_date = $item->get("end_date");
			$count = 0;
			$result = $this->query_result("SELECT count(distinct crm_id) as count from crm_services WHERE crm_id=".$crm_id." and _date between '$start_date' and '$end_date' group by date(_date)", "");
			while ($row = mysql_fetch_array($result)) {
				$count += $row['count'];
			}
		...
		function crm_owner_sum_total_info($owner, $item) {	
			...
			$end_date = $item->get("end_date");
			$count = 0;
			$result = $this->query_result("select sum(amount) as amount from crm_balance where owner='$owner' and _date between '$start_date' and '$end_date' and (deposit='sales' or (product_id=0 and deposit='cash')) and crm_id not in (select crm_id from crm_customer where _class='AGENT')", "");
			while ($row = mysql_fetch_array($result)) {
				$count += abs($row['amount']);
			}
		...
		function crm_customer_first_lease_amount($crm_id, $start_date) {	
			$count = 0;
			$result = $this->query_result("select sum(amount) as amount from crm_balance where crm_id=$crm_id and _date<'$start_date' and deposit='lease'", "");
			while ($row = mysql_fetch_array($result)) {
				$count += abs($row['amount']);
			}
...
//			$where = $this->crm_session_where($where);
		...
		function crm_chart_sales_up_down_list($item) {
			...
			if ($values == "byowner") {
				...
				$wh = " WHERE deposit='sales' and year(_Date)=year(CURRENT_TIMESTAMP) and locate(owner,'$filter')>0 group by month(_date),owner";
				$fds = 'owner';
			}
			if ($values == "byproduct") {
				$q = "SELECT month(_date) as monthName,product_id,sum(".$field.") as amount from crm_balance";
				$wh = " WHERE deposit='sales' and year(_Date)=year(CURRENT_TIMESTAMP) and locate(product_id,'$filter')>0 group by month(_date),product_id";
				$fds = 'product_id';
			}		
		...
		function crm_report_compare_customer_list($item) {
			...
			$owner = $wh[0];
			$product_id = $wh[1];
			$swhere = "WHERE owner='$owner' and YEAR(_date)=YEAR(CURRENT_TIMESTAMP) and crm_id not in (select crm_id from crm_customer where _class='AGENT') GROUP BY owner,crm_id ORDER BY owner,crm_id asc";
		...
		function crm_report_compare_product_user_list($item) {
			...
			$sort = $item->get("sort");
			$dir = $item->get("dir");
			$q = "select product_id,SUM(IF(owner = 'bayarjargal@battrade', ".$field.",0)) as owner1,SUM(IF(owner = 'batchuluun@battrade', ".$field.",0)) as owner2,SUM(IF(owner = 'bayarmaa@battrade', ".$field.",0)) as owner3,SUM(IF(owner = 'dorjsuren@battrade', ".$field.",0)) as owner4,SUM(IF(owner = 'munkhtulga@battrade', ".$field.",0)) as owner5,SUM(IF(owner = 'lodonsharav@battrade', ".$field.",0)) as owner6,SUM(IF(owner = 'battulga@battrade', ".$field.",0)) as owner7,SUM(IF(owner = 'dorjsuren@battrade', ".$field.",0)) as owner8,SUM(IF(owner = 'nyamdorj@battrade', ".$field.",0)) as owner9,SUM(IF(owner = 'otgonchimeg@cosmo', ".$field.",0)) as owner10 from crm_balance where deposit='sales' and crm_id not in (select crm_id from crm_customer where _class='AGENT') and _date between '$start_date' and '$end_date' group by product_id";
		...
		function crm_owner_point_list($owner, $date) {
			...
			$q = "SELECT count(*) as c FROM crm_customer WHERE descr=(select ".$wd." from crm_users where owner='$owner') and level<>'suspect' and crm_id not in (select crm_id from crm_customer where _class='AGENT')";
			$result = $this->query_result($q, "");
			$pow = array();
			while ($row = mysql_fetch_array($result)) {
				$pow['must'] = $row['c'];
			...
			$q = "SELECT count(distinct crm_id) as c FROM crm_services WHERE userCode='$owner' and date(_date)=date('$date') and crm_id not in (select crm_id from crm_customer where _class='AGENT')";
			$result = $this->query_result($q, "");
			while ($row = mysql_fetch_array($result)) {
				$pow['enter'] = $row['c'];
			}
...
//			$where = $this->crm_session_where($where);
		...
		function crm_chart_gps_last_list($item) {
			...
			$query = "delete from crm_gps_box";
			mysql_query($query, $this->db->conn());

			$query = "insert into crm_gps_box (id,owner,lat,lng,_date,crm_id,battery)
			(select id,owner,lat,lng,_date,crm_id,battery from crm_gps where _date>SUBDATE(now(), INTERVAL 2 day))";
			mysql_query($query, $this->db->conn());

			$logged = $_SESSION['logged']['owner'];
			$sort = $item->get("sort");
			$dir = $item->get("dir");
			$q = "SELECT id,owner,(select lat from crm_gps_box where owner=c.owner order by _date desc limit 0,1) as lat,(select lng from crm_gps_box where owner=c.owner order by _date desc limit 0,1) as lng,(select _date from crm_gps_box where owner=c.owner order by _date desc limit 1) as _date from crm_users as c";
			$record = new crm_gps();
			$where = "";
//			$where = $this->crm_where($item, $record->basic_fields);
//			$where = $this->crm_session_where($where);
			...
			if ($user_level == 2) {
				$wr = " WHERE position='Борлуулагч' and user_level=0 and manager='$logged'";
			}else{
				$wr = " WHERE position='Борлуулагч' and user_level=0";
			}
			$count = $this->num_rows($q, $where.$wr);
			$result = $this->query_result($q, $where.$wr);
...
//			$where = $this->crm_where($item, $record->basic_fields);
		...
		function crm_opportunity_by_revenue_list($item) {
			...
			$where = $this->crm_session_where($where);
			$q = "SELECT owner, sum(amount) as actual_revenue, (select sum(amountTheshold) from crm_user_planning where crm_user_planning.owner=crm_balance.owner and start_date between '$start_date' and '$end_date') as target_revenue  from crm_balance ";			
			
			$where .= " and deposit='cash' and _date between '$start_date' and '$end_date' and owner<>'-' and crm_id not in (select crm_id from crm_customer where _class='AGENT') group by owner";
			$result = $this->query_result($q, $where);
			$json = "";
			$count = 0;
			...
			while ($row = mysql_fetch_array($result)) {				
				...
				$owner = $row['owner'];
				$owner = str_replace("@voltam", "", $owner);
				$ab = 0;
				if (floatval($tab) > 0)
